...
exports.view = (req, res) => {
  connection.query(
    'SELECT * FROM student WHERE status = "active" LIMIT 5',
    (err, rows) => {
      if (!err) {
...
exports.views = (req, res) => {
  connection.query(
    "SELECT DISTINCT id  FROM result ORDER BY id LIMIT 5",
    (err, rows) => {
      if (!err) {
        let removedUser = req.query.removed;
...
exports.find = (req, res) => {
  ...
  connection.query(
    "SELECT * FROM student WHERE id LIKE ? OR name LIKE ? OR gender LIKE ? OR department LIKE ?",
    [
      "%" + searchTerm + "%",
      "%" + searchTerm + "%",
      "%" + searchTerm + "%",
...
exports.finds = (req, res) => {
  ...
  connection.query(
    "SELECT DISTINCT id FROM result WHERE id LIKE ?",
    ["%" + searchTerm + "%"],
    (err, rows) => {
      if (rows == 0) {
...
exports.create = (req, res) => {
  ...
  connection.query(
    "INSERT INTO student SET id = ?, name = ?, batch = ?, gender = ?, department = ?, phone = ?, email = ?",
    [id, name, batch, gender, department, phone, email],
    (err, rows) => {
      if (!err) {
...
exports.creates = (req, res) => {
  ...
  connection.query(
    "INSERT INTO result SET id = ?, semester = ?, cgpa = ?",
    [id, semester, cgpa],
    (err, rows) => {
      if (!err) {
...
exports.update = (req, res) => {
  ...
  connection.query(
    "UPDATE student SET id = ?, name = ?, batch = ?, gender = ?, department = ?, phone = ?, email = ? WHERE id = ?",
    [id, name, batch, gender, department, phone, email, req.params.id],
    (err, rows) => {
      if (!err) {
        connection.query(
          "SELECT * FROM student WHERE id = ?",
...
exports.viewalls = (req, res) => {
  ...
  connection.query(
    "SELECT * FROM student NATURAL JOIN result WHERE id = ? ORDER BY semester",
    [req.params.id],
    (err, rows) => {
      if (!err) {
